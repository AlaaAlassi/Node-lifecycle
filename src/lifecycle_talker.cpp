...
public:
  ...
  on_configure(const rclcpp_lifecycle::State &)
  {
    ...
    // available.
     ...
     auto my = this->get_node_parameters_interface();
     int s = my->get_parameter("gain").as_int();
     std::chrono::milliseconds myTime (s);
                ...
                my->get_parameter("gain").get_name().c_str(),
                my->get_parameter("gain").value_to_string().c_str()
              );
    ...
    timer_ = this->create_wall_timer(
      myTime, std::bind(&LifecycleTalker::publish, this));
